...
function buildBaseString(reqScheme, reqMethod, reqDomain, reqPath, reqParams, reqBody, reqAuthHeader) {
  //Formulate the base string
  var baseString = reqMethod.toUpperCase() + "&" +
    reqScheme + "://" + reqDomain + reqPath;

    console.log("querystring params("+(typeof reqParams)+"):", reqParams);
    console.log("headers ("+(typeof reqAuthHeader)+"):", reqAuthHeader);

    //exclude Bearer, exclude
    var baseParams = _.merge(reqAuthHeader, reqParams);

    // Temp remove body
    baseParams = _.merge(baseParams, reqBody);
    baseParams = sortJSON(baseParams);

    var baseParamsStr = querystring.stringify(baseParams);
    baseParamsStr = querystring.unescape(baseParamsStr);

    baseString += "&"+baseParamsStr;
    console.log("Formulated BaseString:" + baseString);
    return baseString;
}
 ...
 * @returns {string}
 */
security.generateSHA256withRSAHeader = function(url, params, method, strContentType, appId, keyCertContent, keyCertPassphrase) {
